...
// [문제1] 1. "소리를 냅니다."라는 추상 메소드 makeSound()를 가진 Soundable 인터페이스를 정의하세요.
interface Soundable{  // 인터페이스 주요 멤버 : 상수 , 추상메소드 : 구현부{}없는 메소드
    public abstract void makeSound(); // public abstract 생략 가능
}
class Cat implements Soundable{  // 오버라이딩 자동완성 : 클래스 안에서 컨트롤+스페이바 -> 방향키 위로 -> 선택후 엔터
...
public class 실습13 {
    public static void main(String[] args) {
        // [문제 1]  main 함수에서 Cat 객체와 Dog 객체를 생성하고, 각 객체의 makeSound() 메소드를 호출하여 결과를 확인하세요.
        new Cat().makeSound();          // 야옹
        Soundable s1 = new Dog();       s1.makeSound(); // 멍멍
        // [문제 2] main 함수에서 객체를 생성하지 않고, RemoteControl.MAX_VOLUME과 같이 인터페이스 이름으로 직접 접근하여 두 상수를 출력하세요.
