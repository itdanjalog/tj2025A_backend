...
// [문제1]  name(문자열) 멤버 변수를 가진 Person 클래스를 만드세요.
class Person{
    // 1. 클래스의 멤버 : 멤버변수 = 특징/상태 , 생성자 = 초기화 , 메소드 = 행동/이벤트
    String name;
}
class Student extends Person { // 2. Person 클래스를 상속(extends연장)받는 Student 클래스를 만드세요.
    String studentId; // Student 클래스에는 studentId(정수) 멤버 변수를 추가하세요.
}
...
class Cat extends Animal{ // makeSound() 메소드를 재정의하여 "고양이가 야옹하고 웁니다."를 출력하는 Cat 클래스를 만드세요.
    void makeSound( ){ System.out.println("고양이가 야옹하고 웁니다.");} // 오버라이딩 조건 : 부모타입의 메소드명의 선언부를 동일하면 된다.
}

public class 실습12 {
    public static void main(String[] args) {
        // [문제 1]  main 함수에서 Student 객체를 생성하고, 상속받은 name과 자신의 studentId에 값을 저장한 뒤 모두 출력하세요.
        Student s1  =  new Student();
        // Student s1 : 변수 , new Student(); : 인스턴스/객체 생성
        s1.name = "유재석";        // 부모의 멤버변수
        s1.studentId = "20113";   // 본인의 멤버변수
        // [문제 2 ]  main 함수에서 Cat 객체를 생성하고 makeSound() 메소드를 호출하여, 재정의된 내용이 출력되는지 확인하세요.
        Cat a1 =  new Cat();        a1.makeSound(); // 고양이가 야옹하고 웁니다.
        Animal a2 = new Cat();      a2.makeSound(); // 고양이가 야옹하고 웁니다.
        Animal a3 = new Animal();   a3.makeSound(); // 동물이 소리를 냅니다.
