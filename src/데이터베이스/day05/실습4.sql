...
-- orders 테이블은 주문이 들어오면 레코드1개(주문1개) 생성
-- 즉] orders 테이블의 총 레코드 개수 ( count 함수 ) 조회( select )
select * from orders;
...
select * from orders;
select * from orders where customer = '김민준'; # customer(고객명) 그룹전에 조건을 사용 = where vs having 
...
select count(*) from books; # 도서가 총 몇권 == 레코드수 == 도서1개당 == 레코드1개  == 레코드 수 구하기 
...
select sum( order_qty ) from orders;	# 총 주문수량의 합계 
...
select * from orders;		# 고객명과 주문수량은 orders 테이블에 있다.
# '김민준'과 '이서연' 고객에 대해 ( 그룹전에 가능한 조건 ) = where 
select * from orders where customer = '김민준' or customer = '이서연'; 	
select * from orders where customer in( '김민준' , '이서연' );
# 각 고객별  = 그룹 
select customer from orders where customer in( '김민준' , '이서연' ) group by customer;
	# 총 주문 수량이 4권 이상인 고객만 조회하세요. ( 그룹/집계 후 가능한 조건 ) = having 
select customer , sum( order_qty ) as 총주문수량 from orders where customer  in( '김민준' , '이서연' ) 
...
select book_id , count(*) as 주문횟수 from orders group by book_id having 주문횟수 >=3 ; # 레코드수가 3개이상인 도서 = having 

-- [문제 9] orders 테이블에서 총 주문 수량이 5권을 초과하는 고객의 customer와 총 주문 수량을 조회하세요.
select customer , sum( order_qty ) as 총주문수량 from orders group by customer having 총주문수량 > 5 ; # 총 주문수량 = having 

-- [문제 10] books 테이블에서 각 저자(author)별로 출판한 도서가 2권 이상인 저자의 이름과 출판 도서 수를 조회하세요.
select author , count(*) as 출판도서수 from books group by author having 출판도서수 >=2;  # 저자별 레코드수==출판한도서수 
