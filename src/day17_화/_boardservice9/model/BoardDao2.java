...
public class BoardDao2 {
    ...
    private BoardDao2(){
        ...
        // 만약에 파일를 로드 하는데.. 파일이 존재하지 않으면
            // [1] 파일 경로에 따른 파일 객체화
        File file = new File("./src/day16_월/_boardservice9/data.txt");
    ...
    public void fileSave(){ // ( 게시물 등록을 성공했을때 지정한 함수 사용/호출 )
        ...
        try {
            // [1] 파일 출력 객체 생성한다.
            FileOutputStream outputStream = new FileOutputStream("./src/day16_월/_boardservice9/data.txt");
    ...
    public void fileLoad( ) { // ( 프로그램이 실행 되었을때 1번 만 실행 ) // DAO 객체(싱글톤) 생성될때 메모장(TXT) 불러오기 한다.
        try {
            // [1] 파일 입력 객체 생성
            FileInputStream inputStream = new FileInputStream("./src/day16_월/_boardservice9/data.txt");
            // [2] 파일 용량만큼 바이트 배열 선언
